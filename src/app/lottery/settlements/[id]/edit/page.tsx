...
export default async function Page({ params }: { params: Promise<{ id: string }> }) { const { id } = await params; const record = findDemoLotterySettlement(Number(id)); if (!record) notFound(); const config = stripCrudSchema(lotterySettlementConfig); return <AppShell><main className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8 lg:py-10"><CrudHeader config={config} mode="edit" id={Number(id)} /><LotterySettlementForm mode="edit" initialRecord={record} /></main></AppShell>; }
