...
export default async function Page({
  ...
  const mapping = demoPosMappings.find((item) => item.id === id);
  if (!mapping) notFound();
  return (
    <AppShell>
      <main className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
        <PosHeader
          title="Edit Mapping"
          description={`Update ${mapping.externalDisplayName ?? mapping.externalEntityKey}.`}
          back="/pos-integration/mappings"
        />
        <PosMappingForm initialValues={mapping} />
      </main>
    </AppShell>
  );
}
