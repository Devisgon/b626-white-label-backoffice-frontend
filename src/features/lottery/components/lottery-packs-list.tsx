...
import { Boxes, Eye, Pencil, Plus, RotateCcw, Search } from "lucide-react";
import { useMemo, useState, type ReactNode } from "react";

import { LotteryPackActionButton } from "@/features/lottery/components/lottery-pack-action-button";
import { demoLotteryPacks } from "@/features/lottery/lottery-pack-demo-data";
import type { LotteryPackStatus } from "@/features/lottery/types";

export function LotteryPacksList() {
  const [search, setSearch] = useState("");

  const [status, setStatus] = useState<LotteryPackStatus | "all">("all");

  const filteredPacks = useMemo(
    () =>
      demoLotteryPacks.filter((pack) => {
        const searchValue = search.trim().toLowerCase();

        const matchesSearch =
          searchValue === "" ||
          pack.pack_number.toLowerCase().includes(searchValue) ||
          pack.game_name.toLowerCase().includes(searchValue);
  ...
  const activePacks = demoLotteryPacks.filter(
    (pack) => pack.status === "Active",
  ).length;

  const inStockPacks = demoLotteryPacks.filter(
    (pack) => pack.status === "In Stock",
  ).length;
  ...
  return (
    <div>
      ...
      >
        <StatCard
          title="Total packs"
          value={String(demoLotteryPacks.length)}
      ...
      >
        ...
        >
          <div>
            <h2 className="font-bold">Lottery packs</h2>

            <p className="mt-1 text-xs text-muted">
              Manage ticket ranges, activation and pack availability.
            </p>
          </div>
        ...
        >
          ...
          <select
            value={status}
            onChange={(event) =>
              setStatus(event.target.value as LotteryPackStatus | "all")
            }
          ...
          >
            <thead className="bg-surface-secondary">
              ...
              >
                ...
                <th className="px-5 py-4">Ticket range</th>
            ...
            <tbody className="divide-y divide-border">
              {filteredPacks.map((pack) => {
                const totalTickets =
                  pack.end_ticket_no - pack.start_ticket_no + 1;
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-5 py-4">{pack.game_name}</td>

                    <td className="px-5 py-4 text-muted">
                      {pack.start_ticket_no} – {pack.end_ticket_no}
                    </td>

                    <td className="px-5 py-4 font-semibold">{totalTickets}</td>
        ...
        {filteredPacks.length === 0 && (
          <div className="px-5 py-14 text-center">
            ...
            <p className="mt-3 font-semibold">No lottery packs found</p>
        ...
        <div
          ...
          "
        >
          Showing {filteredPacks.length} of {demoLotteryPacks.length} packs
        </div>
      </section>
    </div>
  );
