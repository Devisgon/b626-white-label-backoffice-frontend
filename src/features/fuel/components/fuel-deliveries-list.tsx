    ...
    () =>
      data.filter(
        (item) =>
          ...
          `${item.supplier_name} ${item.invoice_number} ${item.tank_name}`
            .toLowerCase()
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-border bg-white shadow-[var(--shadow-sm)]">
      <div className="flex items-center justify-between border-b border-border p-5">
        <div>
          <h2 className="font-bold">Fuel deliveries</h2>
          <p className="mt-1 text-xs text-muted">
            Manage received fuel stock and supplier invoices.
          </p>
        </div>
        <Link
          href="/fuel/deliveries/new"
      ...
      <div className="flex gap-3 border-b border-border p-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search supplier, invoice or tank..."
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[850px] text-left text-sm">
          ...
          <tbody className="divide-y divide-border">
            {rows.map((item) => (
              <tr key={item.id}>
                <td className="p-4 font-semibold">{item.invoice_number}</td>
