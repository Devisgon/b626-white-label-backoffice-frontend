...
const data: FuelSale[] = [
  ...
  },
  {
    ...
    opening_reading: 8300,
    closing_reading: 8520,
    liters_sold: 220,
    price_per_liter: 278.5,
    total_amount: 61270,
    payment_method: "Card",
    shift: "Evening",
    ...
    () =>
      data.filter(
        (item) =>
          ...
          `${item.pump_name} ${item.tank_name} ${item.shift}`
            .toLowerCase()
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-border bg-white shadow-[var(--shadow-sm)]">
      <div className="flex items-center justify-between border-b border-border p-5">
        <div>
          <h2 className="font-bold">Fuel sales</h2>
          <p className="mt-1 text-xs text-muted">
            Track pump readings, liters sold and sale totals.
          </p>
        </div>
        <Link
          href="/fuel/sales/new"
      ...
      <div className="flex gap-3 border-b border-border p-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search pump, tank or shift..."
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[900px] text-left text-sm">
          ...
          <tbody className="divide-y divide-border">
            {rows.map((item) => (
              <tr key={item.id}>
                <td className="p-4 font-semibold">{item.pump_name}</td>
                <td>{item.tank_name}</td>
                <td>{item.liters_sold.toLocaleString()} L</td>
                <td>PKR {item.price_per_liter.toLocaleString()}</td>
