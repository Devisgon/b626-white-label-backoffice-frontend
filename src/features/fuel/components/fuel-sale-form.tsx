...
export function FuelSaleForm({
  mode = "create",
  initialValues,
}: {
  mode?: "create" | "edit";
  initialValues?: Partial<FuelSaleFormInput>;
}) {
  ...
  } = useForm<FuelSaleFormInput>({
    resolver: zodResolver(fuelSaleSchema),
    defaultValues: {
      pump_id: initialValues?.pump_id ?? "",
      tank_id: initialValues?.tank_id ?? "",
      opening_reading: initialValues?.opening_reading ?? "",
      closing_reading: initialValues?.closing_reading ?? "",
  ...
  const liters = Math.max(
    Number(watch("closing_reading") || 0) -
      Number(watch("opening_reading") || 0),
    0,
  );
  const total = liters * Number(watch("price_per_liter") || 0);
  async function onSubmit(values: FuelSaleFormInput) {
    await new Promise((r) => setTimeout(r, 500));
    console.log("Fuel sale payload", {
      ...values,
      pump_id: Number(values.pump_id),
      tank_id: Number(values.tank_id),
      opening_reading: Number(values.opening_reading),
      closing_reading: Number(values.closing_reading),
      liters_sold: liters,
      price_per_liter: Number(values.price_per_liter),
      total_amount: total,
    });
    router.push("/fuel/sales");
  }
  const field =
  ...
  return (
    ...
    >
      <h2 className="text-lg font-bold">Fuel sale information</h2>
      <div className="mt-6 grid gap-5 sm:grid-cols-2">
        <Select
          label="Pump *"
          error={errors.pump_id?.message}
          register={register("pump_id")}
          options={["Pump 01", "Pump 02", "Pump 03"]}
          className={field}
        />
        <Select
          label="Tank *"
          error={errors.tank_id?.message}
          register={register("tank_id")}
          options={["Regular Tank", "Premium Tank", "Diesel Tank"]}
          className={field}
        />
        <Input
          label="Opening reading *"
          error={errors.opening_reading?.message}
          input={
            <input
              type="number"
              step="0.01"
              {...register("opening_reading")}
              className={field}
            />
          }
        />
        <Input
          ...
          error={errors.closing_reading?.message}
          input={
            <input
              type="number"
              step="0.01"
              {...register("closing_reading")}
              className={field}
            />
          }
        />
        ...
        <Input
          ...
          input={
            <input type="date" {...register("sale_date")} className={field} />
          }
        />
        ...
        <div className="rounded-xl bg-primary-light p-4 text-sm">
          <p>
            Liters sold: <b>{liters.toFixed(2)}</b>
          </p>
          <p className="mt-2">
            Total: <b>PKR {total.toLocaleString("en-PK")}</b>
          </p>
        </div>
      </div>
