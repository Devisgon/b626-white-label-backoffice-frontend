...
export async function getProducts(query: ProductQuery = {}) {
  const response = await apiClient.get<ProductListResponse>(PRODUCTS_URL, {
    params: query,
  });
...
export async function getProductCategorySummary() {
  const response = await apiClient.get<ProductCategorySummaryResponse>(
...
export async function getProductHistory(productId: number) {
  const response = await apiClient.get<ProductHistoryResponse>(
...
export async function createProduct(values: CreateProductInput) {
  const response = await apiClient.post<ProductResponse>(PRODUCTS_URL, values);
...
export async function importProducts(file: File) {
  ...
  const response = await apiClient.post<ProductImportResponse>(
    `${PRODUCTS_URL}/import`,
    formData,
    {
      headers: {
