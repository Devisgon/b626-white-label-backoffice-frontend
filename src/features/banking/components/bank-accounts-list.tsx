...
export function BankAccountsList() {
  ...
  const [statusFilter, setStatusFilter] = useState<"all" | BankAccountStatus>(
    "all",
  );
  ...
  const filteredAccounts = useMemo(() => {
    ...
    return accounts.filter((account) => {
      const matchesSearch =
        ...
        account.institution.toLowerCase().includes(normalizedSearch) ||
        account.lastFour.includes(normalizedSearch);
  ...
  function closeAccount(account: BankAccount) {
      ...
      <section className="mt-6 overflow-hidden rounded-2xl border border-border bg-white shadow-[var(--shadow-sm)]">
        <div className="flex flex-col justify-between gap-4 border-b border-border p-5 lg:flex-row lg:items-center">
          <div>
            ...
            <p className="mt-1 text-xs text-muted">
              Manage company accounts, balances and account status.
            </p>
          </div>
        ...
        <div className="grid gap-3 border-b border-border p-5 xl:grid-cols-[minmax(0,1fr)_190px_190px_auto]">
          ...
          <select
            value={statusFilter}
            onChange={(event) =>
              setStatusFilter(event.target.value as "all" | BankAccountStatus)
            }
          ...
          <select
            value={typeFilter}
            onChange={(event) =>
              setTypeFilter(event.target.value as "all" | BankAccountType)
            }
        ...
        {filteredAccounts.length > 0 ? (
          ...
          <div className="p-12 text-center">
            ...
            <h3 className="mt-3 font-semibold">No bank accounts found</h3>
