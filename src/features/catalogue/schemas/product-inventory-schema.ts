...
export const productInventorySchema = z
  .object({
    ...
    location_id: z.coerce
      .number()
      .int("Please select a valid inventory location.")
      .positive("Please select an inventory location."),

    on_hand_quantity: z.coerce
      .number()
      .int("On-hand quantity must be a whole number.")
  ...
  .refine((values) => values.reserved_quantity <= values.on_hand_quantity, {
